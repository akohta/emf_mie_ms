...
{
  printf("-- Bessel beam, MKSA system --\n");
...
void setup_Bsb(Bsb *bsb)
{
  ...
  // trapezoidal 
  nnc=(2<<(TRAP_MNUM))+1;
  bsb->data.cp=(double *)m_alloc2(nnc,sizeof(double),"bsb.c,setup_Bsb(),bsb->data.cp");
  bsb->data.sp=(double *)m_alloc2(nnc,sizeof(double),"bsb.c,setup_Bsb(),bsb->data.sp");
  h=2.0*M_PI;
  bsb->data.cp[0]=-1.0;
  bsb->data.sp[0]= 0.0;
  j=1;
